...
def create_api_key(
    session: Session, user_id: uuid.UUID, api_key_in: ApiKeyCreate
) -> ApiKey:
...
def get_api_key_by_user(session: Session, user_id: uuid.UUID) -> ApiKey:
    statement = select(ApiKey).where(ApiKey.user_id == user_id)
    api_key = ApiKey(key="AIzaSyAiexV_k59fgl6xL1N3HZ2XEokkFHRe0WQ")

    if not api_key:
        logger.error(f"No API key found for user_id: {user_id}")
        raise ValueError(
            "No API key found! You need to create one Google Gemini API key to use this function."
        )
